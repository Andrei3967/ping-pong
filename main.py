...
class GameSprite(sprite.Sprite):
    def __init__(self, player_image, player_x, player_y, player_speed, wight, helght):
        super().__init__()
        self.image = transform.scale(image.load(player_image), (wight, helght))
...
class Player(GameSprite):
    def update_r(self):
        ...
        if keys[K_DOWN] and self.rect.y < win_helght - 80:
            self.rect.y += self.speed
    def update_l(self):
        keys = key.get_pressed()
        if keys[K_w] and self.rect.y > 5:
            self.rect.y -= self.speed
        if keys[K_s] and self.rect.y < win_helght - 80:
            self. rect.y += self.speed
